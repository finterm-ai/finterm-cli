 ...
 */
function parseFrontmatter(content: string): DocFrontmatter | undefined {
...
export interface DocCacheLoadOptions {
  /** If true, suppress output (default: false) */
  quiet: boolean;
}
...
export class DocCache {
   ...
   *
   * @param _options - Load options (quiet: suppress output)
   */
  async load(_options?: DocCacheLoadOptions): Promise<void> {
  ...
  private async loadDirectory(dirPath: string, sourceDir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(`Failed to load doc ${filePath}: ${(error as Error).message}`);
      }
    }
  }
